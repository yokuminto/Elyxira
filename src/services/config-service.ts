...
export interface ConfigExportData {
  ...
  /** 绯想击破 — 仓库配置 */
  breakRepo?: Record<string, unknown>
  /** 绯想击破 — 全部笔记 */
  breakNotes?: Record<string, unknown>
  /** 绯想击破 — 全部标签 */
  breakTags?: Record<string, unknown>
  /** 绯想击破 — 每题答题统计 */
  breakStats?: Record<string, unknown>
  /** 刷题统计 */
  quizStats?: Record<string, unknown>
}
...
/**
 * 合并写入 localStorage Record<string, unknown> 类型的数据：
 * 导入数据覆盖同 key，本地原有 key 保留。
 */
function _mergeLocalStorage(key: string, incoming: Record<string, unknown>): void {
  try {
...
class ConfigService {
  ...
  public getApiPresets(): ApiPreset[] {
    // JSON round-trip avoids structuredClone failure on Vue reactive proxies
  ...
  public exportConfig(): string {
    try {
      ...
      if (quizStats) configToExport.quizStats = JSON.parse(quizStats)
  ...
  public importConfig(configData: unknown): boolean {
    try {
      ...
      if (typedConfig.quizStats) {
        _mergeLocalStorage('quizStats', typedConfig.quizStats as Record<string, unknown>)
      }

      // --- 绯想击破数据：合并写入（导入数据覆盖同 key，本地独有数据保留） ---
      if (typedConfig.breakRepo) {
