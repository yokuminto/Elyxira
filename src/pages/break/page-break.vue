  ...
  <div class="page-break" :style="{ '--font-family': uiSettings.fontFamily || 'sans-serif', '--font-size': (uiSettings.fontSize || 14) + 'px' }">
    ...
    <div v-else class="page-break__container">
      ...
      <header class="page-break__header">
        ...
        <button class="page-break__theme-toggle" :class="{ dark: isDark }" @click="toggleDark" title="切换主题">
...
function playAudio(soundName: 'select' | 'correct' | 'incorrect' | 'navigate') {
  ...
  try {
    ...
    audio.play().catch(e => console.warn('[page-break] audio play failed:', e))
    lastSound.name = soundName
    lastSound.time = now
  } catch (e) { console.warn('[page-break] playSound failed:', e) }
}

const uiSettings = computed(() => configService.getUiSettings())
const isDark = computed(() => uiSettings.value.darkMode)
const toggleDark = () => configService.toggleDarkMode()
...
async function startGame() {
  try {
    ...
    } catch (e) { console.warn('[page-break] getNotes failed:', e) }
