...
function filterThinkStream(chunk: string): { content: string, reasoning: string } {
  let outC = ''; let outR = ''; _thinkBuf += chunk
  const SAFE = 8
  while (_thinkBuf.length > 0) {
    if (!_inThink) {
      const i = _thinkBuf.indexOf('<think>')
      if (i === -1) { const n = Math.max(0, _thinkBuf.length - 6); outC += _thinkBuf.slice(0, n); _thinkBuf = _thinkBuf.slice(n); break }
      outC += _thinkBuf.slice(0, i); _thinkBuf = _thinkBuf.slice(i + 7); _inThink = true
    } else {
      const i = _thinkBuf.indexOf('</think>')
      if (i === -1) { const n = Math.max(0, _thinkBuf.length - SAFE); outR += _thinkBuf.slice(0, n); _thinkBuf = _thinkBuf.slice(n); break }
...
async function renderNotesForCurrentQuestion() {
  ...
  let noteContent = question.notes || '';
...
async function generateAINotes(
  ...
  const handleCompletion: CompletionCallback = async (finalNote, completedIndex, error) => {
    ...
    try {
      ...
      if (currentIndex.value === completedIndex) {
        ...
        await nextTick();
        // 保险：延迟二次渲染确保流式内容完整落盘
        setTimeout(() => renderNotesForCurrentQuestion(), 300);
      }
      if (error) {
  ...
  try {
    ...
    if (requestData.stream && response.body) {
        ...
        if (line.startsWith('data: ')) {
          ...
          if (dataStr !== '[DONE]') {
            try {
              ...
              if (content) {
                const ft = filterThinkStream(content)
                if (ft.content) handleStreamChunk(ft.content, generationTargetIndex)
                if (ft.reasoning) reasoningContent.value += ft.reasoning
              }
            } catch { /* ignore */ }
          }
      ...
      const rawNote = data.choices?.[0]?.message?.content ||
        data.choices?.[0]?.text ||
        data.content?.[0]?.text ||
        '';
      // 过滤 <think> 标签
      ...
      const nonStreamedNote = rawNote.replace(/<think>[\s\S]*?<\/think>/g, '')
      if (questionRef) {
...
function openCurrentQuestionEditor() {
  ...
  questionToEdit.value = structuredClone(currentQuestion.value);
